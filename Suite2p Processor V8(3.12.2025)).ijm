...
ParentD="//Shadowfax/AbbyCui/#54_2.21.25/split/suite2p/"; //the directory of your suite2p folder which contains stabilized files for averaging
//ParentD="C:/Users/yic135/Desktop/#45/split/suite2p/"
...
SDp=20; //number of frames to take of the average for the SD projection, e.g. 40*10= 400frame projection
maxdz=20; //number of frames to take of the average for the SD projection, e.g. 40*10= 400frame projection (both of these are ~2-5 minutes depending)
...
MontageAvg=1; //Montage the average only
tiling=1; //whether to montage as a single column (set to 0) or arrange in a grid, eg 3x2
dz=1; //set this to 1 to make a derivative image of the MONTAGE or 0 to not. If using, it applies a custom LUT whicNormalized.h you'll need to install.
...
for(p=0; p<planes; p++) {
	...
	for (i=0; i<list.length; i++) { 
		...
		if (File.exists(ResidualD+"/residual.tif")) {
			...
			concat="  image1=residual.tif image2="+filename;
		...
		if (frames >= avg){
			...
			close();
			runMacro("Garbage");
			} else{ //if length is shorter than avg, just save the entire chunck as residule and close it
				selectWindow(originalname);
				saveAs("Tiff", ResidualD+"residual");
				close();
			}
